              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                {post.user_id !== user.id && (
                  <button onClick={() => handleFollow(post.user_id, post.username)} style={{ background: followingIds.includes(post.user_id) ? 'rgba(46,166,255,0.15)' : 'rgba(46,166,255,0.9)', color: '#fff', border: 'none', borderRadius: '20px', padding: '2px 10px', fontSize: '11px', cursor: 'pointer', fontWeight: '600' }}>
                    {followingIds.includes(post.user_id) ? '✓' : '+ Подписаться'}
                  </button>
                )}
              </div>
  ...
  if (!user) return (
    <div style={{ minHeight: '100vh', background: '#0e1621', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'SF Pro Display', -apple-system, sans-serif", padding: '20px' }}>
      <div style={{ background: '#17212b', borderRadius: '20px', padding: isMobile ? '32px 24px' : '48px 40px', width: '100%', maxWidth: '360px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
        ...
        <input placeholder="например: durov" value={username} onChange={e => setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g, ''))} style={inputStyle} autoCapitalize="none" autoCorrect="off" />
    ...
    <div style={{ minHeight: '100vh', background: '#0e1621', fontFamily: "'SF Pro Display', -apple-system, sans-serif", color: '#fff' }}>
      ...
      {activeStory && (
        <div onClick={() => setActiveStory(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <img src={activeStory.image_url} style={{ maxHeight: '85vh', maxWidth: isMobile ? '100vw' : '500px', borderRadius: isMobile ? '0' : '16px', objectFit: 'contain' }} />
        </div>
      )}
        ...
        <div style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: '240px', background: '#17212b', borderRight: '1px solid #0d1b29', display: 'flex', flexDirection: 'column', zIndex: 100 }}>
          ...
          <nav style={{ flex: 1, padding: '8px 0', overflowY: 'auto' }}>
            {sideNavItems.map((item: any) => (
              <div key={item.id} onClick={() => setView(item.id as View)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', cursor: 'pointer', borderRadius: '8px', margin: '2px 8px', background: view === item.id ? 'rgba(46,166,255,0.15)' : 'transparent', color: view === item.id ? '#2ea6ff' : '#aab8c2', transition: 'all 0.15s', fontSize: '15px' }}>
      ...
      <div style={{ marginLeft: isMobile ? 0 : '240px', paddingTop: isMobile ? '56px' : 0, paddingBottom: isMobile ? '70px' : 0, minHeight: '100vh' }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#708499', fontSize: '13px', cursor: 'pointer' }}>
                  <span>📎</span>
                  <span>{selectedFile ? selectedFile.name.slice(0, 15) + '...' : 'Фото'}</span>
              ...
              <>
                ...
                {searchResults.users.map(u => (
                  <div key={u.id} style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {u.id !== user.id && (
                      <button onClick={() => handleFollow(u.id!, u.username)} style={{ background: followingIds.includes(u.id!) ? 'rgba(46,166,255,0.15)' : '#2ea6ff', color: '#fff', border: 'none', borderRadius: '20px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: '600' }}>
                        {followingIds.includes(u.id!) ? '✓' : '+ Подписаться'}
                      </button>
                    )}
                  </div>
                ))}
          ...
          <div style={{ maxWidth: '600px', margin: '0 auto', padding: isMobile ? '16px 12px' : '24px 16px' }}>
            ...
            {notifications.map(n => (
              <div key={n.id} style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: '12px', opacity: n.is_read ? 0.6 : 1, borderLeft: n.is_read ? '3px solid transparent' : '3px solid #2ea6ff' }}>
                <span style={{ fontSize: '22px' }}>{n.type === 'like' ? '❤️' : n.type === 'comment' ? '💬' : '👤'}</span>
        ...
        {/* SAVED — only desktop (mobile uses profile) */}
        {view === 'saved' && (
          <div style={{ maxWidth: '600px', margin: '0 auto', padding: isMobile ? '16px 12px' : '24px 16px' }}>
        ...
        {view === 'messages' && (
          <div style={{ display: 'flex', height: isMobile ? 'calc(100vh - 126px)' : '100vh' }}>
            ...
            {(!isMobile || !activeChat) && (
              <div style={{ width: isMobile ? '100%' : '260px', borderRight: isMobile ? 'none' : '1px solid #1e2d3d', background: '#17212b', overflowY: 'auto' }}>
                ...
                {contacts.map(contact => (
                  <div key={contact.id} onClick={() => { setActiveChat(contact); loadMessages(contact.id!) }} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', cursor: 'pointer', background: activeChat?.id === contact.id ? 'rgba(46,166,255,0.1)' : 'transparent', borderLeft: activeChat?.id === contact.id ? '3px solid #2ea6ff' : '3px solid transparent' }}>
                    <div style={{ ...avatarSmall, flexShrink: 0 }}>{getAvatar(contact.username, contact.avatar_url)}</div>
                    <div><div style={{ fontWeight: '600', fontSize: '15px' }}>@{contact.username}</div><div style={{ color: '#708499', fontSize: '12px' }}>{contact.bio || 'нет статуса'}</div></div>
                  </div>
                ))}
              </div>
            )}
                ...
                {!activeChat ? (
                  ...
                  <>
                    <div style={{ padding: '14px 16px', borderBottom: '1px solid #1e2d3d', display: 'flex', alignItems: 'center', gap: '12px', background: '#17212b' }}>
                      {isMobile && (
                        <button onClick={() => setActiveChat(null)} style={{ background: 'none', border: 'none', color: '#2ea6ff', cursor: 'pointer', fontSize: '20px', marginRight: '4px' }}>←</button>
                      )}
                    ...
                    <div style={{ padding: '12px 16px', borderTop: '1px solid #1e2d3d', background: '#17212b', display: 'flex', gap: '10px', alignItems: 'center' }}>
                      <input placeholder="Сообщение..." value={msgText} onChange={e => setMsgText(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} style={{ ...inputStyle, marginBottom: 0, flex: 1 }} />
            ...
            {songs.map((song, i) => (
              <div key={song.id} style={{ ...cardStyle, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '10px', background: `linear-gradient(135deg, hsl(${i * 60}, 70%, 40%), hsl(${i * 60 + 40}, 70%, 30%))`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>🎵</div>
                  <div style={{ flex: 1 }}><div style={{ fontWeight: '600' }}>{song.title}</div><div style={{ color: '#708499', fontSize: '13px' }}>{song.artist}</div></div>
                </div>
        ...
        {view === 'profile' && (
          <div style={{ maxWidth: '500px', margin: '0 auto', padding: isMobile ? '16px 12px' : '24px 16px' }}>
            ...
            <div style={cardStyle}>
              ...
              {/* Mobile only: saved & music shortcuts */}
              {isMobile && (
                <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                  <button onClick={() => setView('saved')} style={{ flex: 1, padding: '10px', background: '#1e2d3d', border: 'none', borderRadius: '12px', color: '#aab8c2', cursor: 'pointer', fontSize: '13px' }}>🔖 Сохранённые</button>
                  <button onClick={() => setView('music')} style={{ flex: 1, padding: '10px', background: '#1e2d3d', border: 'none', borderRadius: '12px', color: '#aab8c2', cursor: 'pointer', fontSize: '13px' }}>🎵 Музыка</button>
                </div>
              )}
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      {isMobile && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '60px', background: '#17212b', borderTop: '1px solid #1e2d3d', display: 'flex', alignItems: 'center', justifyContent: 'space-around', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom)' }}>
          {navItems.map((item: any) => (
            <button key={item.id} onClick={() => setView(item.id as View)} style={{ background: 'none', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', cursor: 'pointer', padding: '6px 12px', position: 'relative', opacity: view === item.id ? 1 : 0.5 }}>
              <span style={{ fontSize: '22px' }}>{item.icon}</span>
              <span style={{ fontSize: '10px', color: view === item.id ? '#2ea6ff' : '#708499', fontWeight: view === item.id ? '700' : '400' }}>{item.label}</span>
              {item.badge > 0 && <span style={{ position: 'absolute', top: '2px', right: '6px', background: '#f87171', color: '#fff', borderRadius: '10px', padding: '0 5px', fontSize: '10px', fontWeight: '700' }}>{item.badge}</span>}
            </button>
          ))}
        </div>
      )}
